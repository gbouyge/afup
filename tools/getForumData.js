...
casper.start("http://afup.org/pages/forumphp2014/sessions.php", function() {
    this.capture('forumphp2014.png');
	...
	all_confs = this.evaluate(function() {
			...
			function getDate(dateString,isStart) {
				...
				function conferencier(name, image, link) {
					...
					this.img = image;
					this.link = 'http://afup.org/pages/forumphp2014/' + link;
				}
			...
			$( ".session" ).each(function( index,self ) {
				...
				dateEl = $(self).find(".horaire");
				dateEl.find('span').remove();
				date = dateEl.text();
				date = date.replace("-", " - ");
